...
describe("Password Validation", () => {
  describe("validatePasswordStrength", () => {
    it("should reject passwords shorter than minimum length", async () => {
      ...
      expect(result.feedback.warning).toContain(
        `at least ${PASSWORD_MIN_LENGTH} characters`,
      );
    });

    it("should reject weak passwords (score < 3)", async () => {
      const result = await validatePasswordStrength("password123");
      expect(result.isValid).toBe(false);
      expect(result.score).toBeLessThan(PASSWORD_MIN_SCORE);
    });

    it("should accept strong passwords (score >= 3)", async () => {
      const result = await validatePasswordStrength("MyStr0ng!P@ssw0rd2024");
      expect(result.isValid).toBe(true);
      expect(result.score).toBeGreaterThanOrEqual(PASSWORD_MIN_SCORE);
    });

    it("should detect password based on user inputs", async () => {
      const email = "alice@example.com";
      const name = "Alice";

      // Password containing user info should be weak
      const result1 = await validatePasswordStrength("alice12345", [
        email,
        name,
      ]);
      ...
      const result2 = await validatePasswordStrength("C0mpl3xP@ssw0rd!", [
        email,
        name,
      ]);
    ...
    it("should provide helpful feedback for weak passwords", async () => {
      const result = await validatePasswordStrength("abcdefghij");
      expect(result.isValid).toBe(false);
    ...
    it("should handle edge cases", async () => {
      // Empty password
      const result1 = await validatePasswordStrength("");
      expect(result1.isValid).toBe(false);

      // Very long strong password
      const result2 = await validatePasswordStrength(
