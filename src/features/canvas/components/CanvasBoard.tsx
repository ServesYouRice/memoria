...
import { Alert, Box, Button, SpeedDial, SpeedDialAction, SpeedDialIcon } from '@mui/material';
import { NoteAdd, Bookmark, Image as ImageIcon, Poll } from '@mui/icons-material';
import { useSession } from 'next-auth/react';
import { useDeleteCanvasItem, useCreateCanvasItem, useUpdateCanvasItem, canvasItemKeys } from '@/lib/hooks/use-canvas-items';
...
export function CanvasBoard({ canvasId }: CanvasBoardProps) {
    ...
    const { users: collaborators, cursors, connected: collaborationConnected, status: collaborationStatus, broadcastMessage } = useCollaboration({
        canvasId,
    ...
    const handleDragEnd = (e: Konva.KonvaEventObject<DragEvent>, item: CanvasItem) => {
        updateItem({
            itemId: item.id,
            data: {
    ...
    const handleContextMenu = (e: React.MouseEvent | Konva.KonvaEventObject<MouseEvent>, itemId: string) => {
        if ('evt' in e) {
    ...
    const handleItemChange = (id: string, data: Partial<Pick<CanvasItem, 'positionX' | 'positionY' | 'width' | 'height' | 'content' | 'zIndex'>>) => {
    ...
    // Virtualization
    const renderedItems = useVirtualItems(items.map(item => ({
        ...item,
        x: item.positionX,
        y: item.positionY,
    })), {
    ...
    return (
        <Box ref={containerRef} sx={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
            <CanvasHeader
                ...
                onFitToScreen={() => { setZoom(1); setPosition({ x: 0, y: 0 }); }}
            ...
            {canvasLoadError && (
                <Box sx={{ px: 2, py: 1 }}>
                    <Alert
                        severity="error"
                        action={(
                            <Button color="inherit" size="small" onClick={() => refreshMetadata()}>
                                Retry
                            </Button>
                        )}
                        onClose={() => { }}
            ...
            <Box sx={{ flexGrow: 1, position: 'relative', overflow: 'hidden', cursor: isDrawing ? 'crosshair' : 'default', bgcolor: '#f0f2f5' }}>
                {!isDrawing && !isPresentationMode && (
                    ...
                    >
                        ...
                        <SpeedDialAction icon={<ImageIcon />} tooltipTitle="Image" onClick={() => setImageDialogOpen(true)} />
