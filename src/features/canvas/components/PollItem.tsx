...
export function PollItem({ item, isSelected, onSelect, onContextMenu }: PollItemProps) {
    ...
    const height = headerHeight + options.length * (optionHeight + optionGap) + padding * 2;
    ...
    const handleVote = (optionId: string) => {
        ...
        const newOptions = options.map((opt) => {
            ...
            if (opt.id === optionId) {
                ...
                const isAddingVote = targetOption && !targetOption.votes.includes(userId);
    ...
    return (
        ...
        >
            ...
            {options.map((option, index) => {
                ...
                const percent = totalVotes > 0 ? (option.votes.length / totalVotes) * 100 : 0;
