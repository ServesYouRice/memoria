...
interface UseCanvasChatOptions {
    ...
    onMessageReceived?: (callback: (message: ChatMessage | ReactionMessage) => void) => void;
}
...
export function useCanvasChat({
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setRemoteReactions(prev => prev.filter(r => now - r.timestamp < 3000));
        }, 1000);
    ...
    const sendChatMessage = useCallback((content: string, canvasX: number, canvasY: number) => {
    ...
    const sendReaction = useCallback((emoji: string, canvasX: number, canvasY: number) => {
        if (!broadcastMessage) return;
