...
export function hasActiveDialog(): boolean {
  return (
    ...
    Boolean(document.querySelector('[role="dialog"]:not([aria-hidden="true"])'))
  );
}

export function shouldIgnoreGlobalShortcut(
  event: Pick<KeyboardEvent, "target">,
  options: { allowInEditable?: boolean; allowInDialog?: boolean } = {},
): boolean {
  if (!options.allowInEditable && isEditableEventTarget(event)) return true;
  if (!options.allowInDialog && hasActiveDialog()) return true;
  return false;
}
