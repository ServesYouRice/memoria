...
export function useItemConnections(canvasId: string | undefined) {
    return useQuery<ConnectionsResponse>({
        ...
        queryFn: async () => {
            const response = await fetch(`/api/v1/canvases/${canvasId}/connections`);
...
export function useCreateConnection() {
    ...
    return useMutation({
        ...
        onMutate: async ({ canvasId, fromId, toId, label, style }) => {
            await queryClient.cancelQueries({ queryKey: connectionKeys.canvas(canvasId) });
            const previous = queryClient.getQueryData<ConnectionsResponse>(connectionKeys.canvas(canvasId));

            if (previous) {
                queryClient.setQueryData<ConnectionsResponse>(connectionKeys.canvas(canvasId), {
                    connections: [
        ...
        onError: (_err, { canvasId }, context) => {
            if (context?.previous) {
                queryClient.setQueryData(connectionKeys.canvas(canvasId), context.previous);
            }
        },
        onSettled: (_data, _error, { canvasId }) => {
            queryClient.invalidateQueries({ queryKey: connectionKeys.canvas(canvasId) });
        },
    });
}
...
export function useUpdateConnection() {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            const response = await fetch(`/api/v1/canvases/${canvasId}/connections/${connectionId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
...
export function useDeleteConnection() {
    ...
    return useMutation({
        ...
        onMutate: async ({ canvasId, connectionId }) => {
            ...
            if (previous) {
                queryClient.setQueryData<ConnectionsResponse>(connectionKeys.canvas(canvasId), {
                    connections: previous.connections.filter((c) => c.id !== connectionId),
                });
            }
