...
test.describe("Canvas Sharing", () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // Create a canvas
    await page.click(
      'button:has-text("New Canvas"), button:has-text("Create")',
    );
  ...
  test.describe("Create Share Link", () => {
    test("should create a view-only share link", async ({ page }) => {
      ...
      // Create share link
      await page.click(
        'button:has-text("Create link"), button:has-text("Generate")',
      );
    ...
    test("should create an edit share link", async ({ page }) => {
      ...
      // Should show share link
      await expect(
        page.locator('input[readonly][value*="/share/"]'),
      ).toBeVisible({
        timeout: 5000,
      });
    });
  ...
  test.describe("Access Shared Canvas", () => {
    test("should access view-only shared canvas without login", async ({
      page,
      context,
    }) => {
      ...
      // Should load canvas
      await expect(
        page.locator('[data-testid="canvas-stage"], canvas'),
      ).toBeVisible({
        timeout: 10000,
      });
      ...
      await expect(
        page.locator('button:has-text("Edit"), button:has-text("Delete")'),
      ).not.toBeVisible();
    ...
    test("should prevent editing in view-only mode", async ({
      page,
      context,
    }) => {
      await context.clearCookies();
      ...
      await expect(
        page.locator('dialog:has-text("Add"), dialog:has-text("Create")'),
      ).not.toBeVisible();
    });
    ...
    test("should allow editing in edit mode", async ({ page, context }) => {
      ...
      await ownerPage.click(
        'button[aria-label="Share"], button:has-text("Share")',
      );
      ...
      const editShareUrl = await ownerPage
        .locator('input[readonly][value*="/share/"]')
        .inputValue();
  ...
  test.describe("Manage Share Links", () => {
    test("should list all active share links", async ({ page }) => {
      ...
      await expect(
        page.locator('li:has-text("view"), li:has-text("edit")'),
      ).toBeVisible();
    });
    ...
    test("should revoke share link", async ({ page }) => {
      ...
      await page
        .click('button:has-text("Revoke"), button:has-text("Delete")')
        .first();
    ...
    test("should prevent access after revoking share link", async ({
      page,
      context,
    }) => {
      ...
      const shareUrlInput = page
        .locator('input[readonly][value*="/share/"]')
        .first();
      const revokedShareUrl = await shareUrlInput.inputValue();
      const revokedTokenMatch = revokedShareUrl.match(
        /\/share\/([a-zA-Z0-9_-]+)/,
      );
      ...
      await expect(
        newPage.locator("text=/not found|invalid|expired|revoked/i"),
      ).toBeVisible({
        timeout: 5000,
      });
  ...
  test.describe("Public Canvas", () => {
    test("should make canvas public", async ({ page }) => {
      ...
      await expect(
        page.locator('input[readonly][value*="/public/"]'),
      ).toBeVisible({
        timeout: 5000,
      });
    });

    test("should access public canvas without login", async ({
      page,
      context,
    }) => {
      ...
      const publicUrl = await ownerPage
        .locator('input[readonly][value*="/public/"]')
        .inputValue();
      await ownerPage.close();
  ...
  test.describe("Share Link Expiration", () => {
    test("should not access expired share link", async ({ page, context }) => {
      ...
      const expiredShareUrl = await page
        .locator('input[readonly][value*="/share/"]')
        .inputValue();
      const expiredTokenMatch = expiredShareUrl.match(
        /\/share\/([a-zA-Z0-9_-]+)/,
      );
      ...
      await expect(
        newPage.locator("text=/expired|no longer available/i"),
      ).toBeVisible({
        timeout: 5000,
      });
  ...
  test.describe("Share Permissions", () => {
    test("should only allow owner to create shares", async ({ page }) => {
      // Create another user
      const otherUser = {
      ...
      await expect(
        page.locator("text=/not found|access denied|forbidden/i"),
      ).toBeVisible({
        timeout: 5000,
      });
    });
