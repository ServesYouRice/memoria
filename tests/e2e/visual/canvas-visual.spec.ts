...
test.describe('Canvas Visual Regression', () => {
    ...
    itPercy('empty canvas state', async ({ page }) => {
        ...
        await page.waitForSelector('[data-testid="canvas-stage"], .konvajs-content', {
            timeout: 10000,
        });
    ...
    itPercy('canvas with note items', async ({ page }) => {
        ...
        const addNoteBtn = page.locator('[data-testid="add-note-button"], button:has-text("Note")');
    ...
    itPercy('canvas zoom states', async ({ page }) => {
        ...
        // Zoom out
        const zoomOutBtn = page.locator('[data-testid="zoom-out"], button[aria-label*="zoom out" i]');
