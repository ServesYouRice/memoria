...
test.describe('Canvas CRUD Operations', () => {
  ...
  test('should display empty state when no canvases exist', async ({ page }) => {
    await page.goto('/canvases');

    // Should show empty state or create canvas button
    const createButton = page.getByRole('button', { name: /create|new canvas/i });
  ...
  test('should display list of canvases', async ({ page }) => {
    ...
    const canvasList = page.locator('[role="list"], .canvas-list, .canvases-grid');
  ...
  test('should update canvas name', async ({ page }) => {
    ...
    if (await canvasLink.isVisible()) {
      ...
      const editButton = page.getByRole('button', { name: /edit|settings|rename/i });
      ...
      if (await editButton.isVisible()) {
        ...
        // Verify name changed
        await expect(page.getByText('Updated Canvas Name')).toBeVisible({ timeout: 3000 });
      }
    }
  });
...
test.describe('Canvas Permissions', () => {
  ...
  test('should not allow accessing other users canvases', async ({ page }) => {
    ...
    const isRedirected = page.url().includes('/canvases') || page.url().includes('/404');
