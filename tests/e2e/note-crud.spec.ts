...
test.describe("Note CRUD Operations", () => {
  ...
  test("should handle note creation when API is unavailable", async ({
    page,
  }) => {
  ...
  test("should show error message when items fail to load", async ({
    page,
  }) => {
      ...
      if (route.request().method() === "PATCH") {
        route.fulfill({
          ...
          body: JSON.stringify({
            ...
            status: 409,
            detail:
              "Version mismatch. Expected version 1, but current version is 2.",
          }),
        });
      } else {
        route.continue();
...
test.describe("Viewport-Based Pagination", () => {
  ...
  test("should fetch items without viewport params (backwards compatibility)", async ({
    page,
  }) => {
  ...
  test("should support viewport-based filtering with minX, maxX, minY, maxY", async ({
    ...
    expect(viewportRequest).not.toBeNull();
    expect(viewportRequest?.searchParams.get("minX")).toBe("0");
    expect(viewportRequest?.searchParams.get("maxX")).toBe("1000");
    expect(viewportRequest?.searchParams.get("minY")).toBe("0");
    expect(viewportRequest?.searchParams.get("maxY")).toBe("800");
  ...
  test("should return empty items for viewport with no items", async ({
    page,
  }) => {
  ...
  test("should work with type filter alongside viewport params", async ({
    page,
  }) => {
    const canvasId = "demo-canvas-id";
